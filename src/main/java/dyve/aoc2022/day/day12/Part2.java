...
public class Part2 extends Part {
    ...
    protected Object execute(InputReader reader) {
        ...
        Dijkstra.calculateShortestPathFromSource(graph, nodes.get(end));

        return map.entrySet().stream().filter(e -> e.getValue() == Height.a).map(e -> nodes.get(e.getKey())).mapToInt(Node::getDistance).min().orElseThrow();
    }
